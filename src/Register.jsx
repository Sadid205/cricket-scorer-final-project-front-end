...
<form onSubmit={(e)=>handleSubmit(e)} style={{height:"100vh"}} className="flex bg-gray-700 justify-center items-center space-y-8">
    ...
    <label for="username" className={`font-bold text-lg ${signUpResponse?(signUpResponse.username?("text-red-800"):("text-white")):("text-white")}`}>{signUpResponse?(signUpResponse.username?(signUpResponse.username[0]):("Username")):("Username")}</label> 
      <input value={username} onChange={(e)=>setUsername(e.target.value)} type="text" name="username" placeholder="Username" className="border rounded-lg py-3 px-3 mt-4 bg-black border-indigo-600 placeholder-white-500 text-white" />
      <label for="first_name" className="font-bold text-lg text-white">First Name</label> 
      <input value={firstName} onChange={(e)=>setFirstName(e.target.value)} type="text" name="first_name" placeholder="First Name" className="border rounded-lg py-3 px-3 bg-black border-indigo-600 placeholder-white-500 text-white" />
      <label for="last_name" className="font-bold text-lg text-white">Last Name</label> 
      <input value={lastName} onChange={(e)=>setLastName(e.target.value)} type="text" name="last_name" placeholder="Last Name" className="border rounded-lg py-3 px-3 bg-black border-indigo-600 placeholder-white-500 text-white" />
      ...
      <input value={email} onChange={(e)=>setEmail(e.target.value)} type="email" name="email" placeholder="Email" className="border rounded-lg py-3 px-3 mt-4 bg-black border-indigo-600 placeholder-white-500 text-white" />
      ...
      <input value={password} onChange={(e)=>setPassword(e.target.value)} type="password" name="password" placeholder="Password" className="border rounded-lg py-3 px-3 mt-4 bg-black border-indigo-600 placeholder-white-500 text-white" />
      ...
      <input value={confirmPassword} onChange={(e)=>setConfirmPassword(e.target.value)} type="password" name="confirm_password" placeholder="Confirm Password" className="border rounded-lg py-3 px-3 mt-4 bg-black border-indigo-600 placeholder-white-500 text-white" />
      <button type="submit" className="border mt-4 border-indigo-600 bg-black text-white rounded-lg py-3 font-semibold">Sign Up</button>
    </div>
</form>
<div>
<ToastContainer />
